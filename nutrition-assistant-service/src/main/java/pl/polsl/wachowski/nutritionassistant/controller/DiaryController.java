...
public class DiaryController {
    ...
    ResponseEntity<DiaryEntriesResponse> getEntries(@PathVariable(DATE) final LocalDate diaryDate) {
    ...
    ResponseEntity<Void> addEntry(@PathVariable(DATE) final LocalDate diaryDate,
    ...
    @PatchMapping(path = ENTRIES_POSITION_API_SUFFIX,
                  consumes = APPLICATION_JSON_VALUE)
    ResponseEntity<Void> editEntry(@PathVariable(DATE) final LocalDate diaryDate,
    ...
    ResponseEntity<Void> deleteEntry(@PathVariable(DATE) final LocalDate diaryDate,
                                     @PathVariable(ENTRY_POSITION) final short entryPosition) {
    ...
    @PatchMapping(path = ENTRIES_API_SUFFIX,
                  consumes = APPLICATION_JSON_VALUE)
    ResponseEntity<Void> moveEntry(@PathVariable(DATE) final LocalDate diaryDate,
